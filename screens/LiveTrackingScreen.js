...
export default function LiveTrackingScreen() {
  ...
  useEffect(() => {
    ...
    const initLocation = async () => {
      try {
        ...
        subscription = await Location.watchPositionAsync(
          ...
          (loc) => {
            if (loc?.coords) {
              ...
              // Update route live if customer selected
              if (selectedCustomer) {
                fetchRoute(
        ...
        Alert.alert("Error", "Failed to get location. Please restart the app.");
      }
    };
  ...
  const fetchRoute = async (origin, destination) => {
    try {
      ...
      console.log("Google Directions API error:", error);
    }
  };
  ...
  const handleTrack = () => {
    ...
    fetchRoute(currentLocation, selectedCustomer);
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (mapRef.current && currentLocation) {
                  fetchRoute(currentLocation, item);
